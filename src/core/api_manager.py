    ...
    def request(self, req: ApiRequest, max_retries: int = 3):
        ...
        while attempt <= max_retries:
            ...
            try:
                ...
                wait = self.limiter.record_429(req.provider, getattr(exc, "retry_after", None))
...
class RateLimited(Exception):
    """Raised by a provider client on HTTP 429. Carries the provider's
    Retry-After value (seconds) when it sent one, so the rate limiter can
    honor it exactly instead of guessing a backoff."""
